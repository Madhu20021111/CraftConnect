...
export default function ArtisanArtworksPage() {
  ...
  useEffect(() => {
    const fetchArtworks = async () => {
      ...
      try {
        const res = await api.get(`/products/my-artworks`);
  ...
  const handleDelete = async (id: string | number) => {
    ...
    try {
      ...
      setMyArtworks(prev => prev.filter(art => art.id !== id));
    } catch (error) {
          ...
          {myArtworks.map((art, i) => (
            <div key={art.id} className="flex flex-col h-full relative group">
              ...
              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                <Link href={`/dashboard/artworks/edit/${art.id}`}>
                ...
                <button 
                  ...
                  className="w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full text-red-500 flex items-center justify-center hover:bg-red-50 shadow-sm border border-craft-border"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
